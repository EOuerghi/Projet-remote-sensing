...
def writeGTIFF(im, fname, copy_metadata_from=None):
    ...
    with rasterio.open(fname, 'w', **p) as d:
            ...
            d.update_tags(date=acquisition_date(copy_metadata_from))
            

    #with rasterio.open(fname,'w', **p) as src :
        #src.update_tags(date=acquisition_date(copy_metadata_from))
...
def acquisition_date(geotiff_path):
    ...
    with rasterio.open(geotiff_path, 'r') as src:
        if 'NITF_IDATIM' in src.tags():
            ...
            return datetime.datetime.strptime(date_string, "%Y%m%d%H%M%S")
        elif 'TIFFTAG_DATETIME' in src.tags():
            date_string = src.tags()['TIFFTAG_DATETIME']
            date_string = date_string[:4] + '-' + date_string[5:7] + '-' + date_string[8:10] 
            return date_string
        else :
            date_string = src.tags()['date']
            return(date_string)
